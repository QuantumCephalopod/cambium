...
function inspect(path,source='global-minimap'){if(!nav||!nav.inspect(path))return false;emitView(source);drawTwin();if(isPhilosophySource(source))scheduleSwingback(140);return true}
...
globalThis.SSSWorldView=Object.freeze({setScope,inspect,clearInspection,rotateBy,drawTwin,swingback,get scopeId(){return scopeId},get projection(){return projection},get view(){return nav?.view||''},get language(){return lang},get orientation(){return [...orient]},get structure(){return STRUCT},projectPoint(p,rect,scale=1.72){const q=qRot(orient,p),f=(Math.min(rect.width,rect.height)/2)/Math.tan(Math.PI/6.6),z=3.0-q[2]*scale;return {x:rect.width/2+q[0]*scale*f/z,y:rect.height/2-q[1]*scale*f/z,z:q[2]}}});
})();
