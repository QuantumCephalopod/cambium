...
function reconcile(){pending=null;commit.classList.remove('show');if(!W.view)return;const r=resolveGlobal(W.view);if(!r.interlocutors.length)return;const ids=r.interlocutors.map(x=>x.interlocutorId);if(W.view===activeAddress&&sameIds(ids,activeIds))return;pending={resolved:r,path:W.view};const names=ids.map(x=>x.replace(/^organism:/,''));commit.textContent=(W.language==='de'?'Locus öffnen · ':'open locus · ')+names.join(' + ');commit.classList.add('show')}
