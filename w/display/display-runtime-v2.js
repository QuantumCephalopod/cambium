...
/* Philosophy background inspection remains intentionally two-step. The global
 * minimap never uses this pending/commit path: it dispatches direct navigation. */
function reconcile(){
...
function navigateGlobal(path,push=true,origin=null){
  ...
  fold.swap(()=>enter(r,path,push),{origin,from:activeAddress,to:path});
  return true;
}
...
home.addEventListener('click',e=>{e.preventDefault();if(activeAddress||!activeIds.includes('organism:philosophy'))navigateGlobal('',true,{x:innerWidth/2,y:innerHeight/2});else setLocalView('','home')});
