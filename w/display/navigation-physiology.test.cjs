...
assert.strictEqual('AXIS_LATCH_MS' in N, false, 'withdrawn latch experiment must not remain navigation law');
assert.strictEqual('axisLatch' in state, false, 'withdrawn latch state must not remain navigation anatomy');

console.log(JSON.stringify({status:'pass',realized_only:true,independent_axes:true,inspect_commit:true,axis_latch:'withdrawn/unearned'}, null, 2));
