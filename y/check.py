...
def main():
    ...
    check("PRETEXT_ID='@chenglou/pretext'" in papers_sierpinski and "state.dependency(PRETEXT_ID,'layout.js')" in papers_sierpinski and "PRETEXT_VERSION='0.0.9'" in papers_sierpinski and 'prepareWithSegments' in papers_sierpinski and 'layoutNextLineRange' in papers_sierpinski and 'materializeLineRange' in papers_sierpinski and 'preparedWisdom(entity,volumeText,font)' in papers_sierpinski,'Papers canonical selected-organism 1T text is not resolving pinned Pretext by stable Display identity')
