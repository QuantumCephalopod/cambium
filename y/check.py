...
def main():
    ...
    check('projection?.source_meta?.[x.id]' in papers_sierpinski and 'EXTERNAL ORIGIN' in papers_sierpinski and 'COMPOSITION' in papers_sierpinski and 'PAPERS ORGANISM' in papers_sierpinski and 'papers-source-inquiry' in papers_sierpinski,'Papers selected-organism inquiry lost origin-specific provenance/genealogy evidence')
    check('function inquiryBody(id)' in papers_sierpinski and 'function inquiryMetabolites(entity)' in papers_sierpinski and 'function inquiryWisdomText(entity)' in papers_sierpinski and 'function organismInquiryReceipt(id)' in papers_sierpinski and 'function updateOrganismInquiry()' in papers_sierpinski and 'TETRAHEDRAL BODY · PROJECTION GAP' in papers_sierpinski and 'function updateSourceInquiry()' not in papers_sierpinski,'Papers still forks Source/Holon into different inquiry display architectures')
    ...
    check('drawLights' in papers_sierpinski and 'metabolight' in papers_sierpinski and 'if(entity)lights.push(metabolight' in papers_sierpinski and "kind:rank===0?'quantum':'metabolight'" in papers_sierpinski and 'quantumEmberCount' in papers_sierpinski,'Papers organism light law is no longer rank-driven/self-similar')
    check("PRETEXT_ID='@chenglou/pretext'" in papers_sierpinski and "state.dependency(PRETEXT_ID,'layout.js')" in papers_sierpinski and "PRETEXT_VERSION='0.0.9'" in papers_sierpinski and 'prepareWithSegments' in papers_sierpinski and 'layoutNextLineRange' in papers_sierpinski and 'materializeLineRange' in papers_sierpinski and 'preparedWisdom(entity,wisdomText,font)' in papers_sierpinski,'Papers canonical selected-organism metabolite text is not resolving pinned Pretext by stable Display identity')
    check("m.compression" in papers_sierpinski and "join('   ◆   ')" in papers_sierpinski and "canvas.dataset.wisdomSource='metabolites'" in papers_sierpinski and 'canvas.dataset.wisdomMetabolites=String(metabolites.length)' in papers_sierpinski and 'function inquiryVolumeText(entity)' not in papers_sierpinski and 'preparedWisdom(entity,volumeText,font)' not in papers_sierpinski,'Papers is again presenting 1T derivation as primary visible wisdom instead of surviving metabolites')
