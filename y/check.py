...
def main():
    ...
    check("viewTarget=null" in fields and "typeof viewTarget==='function'" in fields and 'viewTargetExternal' in fields,'generic field external view target hook missing')
    ...
    check('OVERVIEW_LIGHT_GAIN=.46' in papers_sierpinski and 'INQUIRY_LIGHT_GAIN=.72' in papers_sierpinski and 'drawLights(population.lights' in papers_sierpinski,'Papers light gain correction regressed')
    check('PARENT_FIELD_ID' in papers_sierpinski and "modules.get('organism:philosophy')" in papers_sierpinski and 'projection=W?.projection' in papers_sierpinski and 'inspectable:false,draggable:false' in papers_sierpinski,'Papers parent inquiry off-field is not live Philosophy-owned tissue')
    check("PARENT_MOUNT_PATH='y'" in papers_sierpinski and 'parentMountFrame' in papers_sierpinski and 'parentInquiryViewTarget' in papers_sierpinski and 'viewTarget:parentInquiryViewTarget' in papers_sierpinski and 'local.scale/frame.scale' in papers_sierpinski,'Papers parent field is not coupled through the scoped global-y mount frame')
    check('parentMountPath' in papers_sierpinski and 'localChamberPath' in papers_sierpinski and 'parentViewScale' in papers_sierpinski,'Papers parent/local coupled frame is not observable')
    check('.papers-parent-field-stage' in papers_sierpinski_css and 'pointer-events:none' in papers_sierpinski_css,'Papers Philosophy parent field is not passive deep-off tissue')
