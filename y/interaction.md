# named places — current interaction refinement

## pressure

the first rendered trial exposed three usability problems: letter labels hid the
meaning already carried by the derived nouns; multiple public navigation modes
competed with the actual website; and the camera could turn only horizontally.
Later use added two visitor-facing wounds: the top navigation loses useful relational
movement after entering a place, and mobile touch can fight scrolling/rotation.

This note carries the current host-orientation/renewal consequence. Visitor-facing
pressure itself now enters the `display` organ's stomach rather than being treated as
host semantic anatomy.

## consequence

- one production navigator consumes a runtime projection derived from host
  `INDEX.yaml + _cambium.yaml`; it never treats the minimal phenotype as a flat node
  table or file registry.
- visitor-facing wording/template/style/view now live intact in `display/`, placed as
  one organ at host `w / expression`. They are not scattered across host `w/x/z/y`
  carrier folders to mimic the root phenotype.
- host `z` retains exact address identity, route admission and interaction control;
  host `y` retains build/witness/regeneration. Display consumes those across its
  interface without inheriting them as inner descendants.
- each breadcrumb label is the locally earned noun at its complete raw prefix; the
  URL retains the symbolic witness beneath mutable names.
- paired ancestry rendering remains available. Geometric quotient may identify one
  place without erasing raw genealogy; semantic coalescence appears only when
  independently realized witnesses carry the same semantic whole.
- quaternion arcball rotation supports horizontal/vertical turning and roll;
  keyboard arrows, q/e and reset remain non-drag controls. Camera state is separate
  from address/browser history.
- `y/specimen.json` remains synthetic recursive navigation tissue only; it never
  contributes production phenotype or display anatomy.

## display boundary

`display/INDEX.yaml` is currently `{}` and there is no `display/_cambium.yaml`.
That is deliberate: display is a real independently re-enterable organ, but its
internal CCCC differentiation has not yet been earned. Current page sources are one
unsplit body.

The generated `_site/` Pages artifact is a secreted membrane, not living renewal or
display tissue. Root `index.html` is no longer committed.

## witness scope

core checks are rerunnable through:

```sh
python3 y/build.py --artifact _site
python3 y/check.py --artifact _site
SITE_DIR=_site node y/test-address.cjs
```

`browser-check.py` remains the optional rendered interaction witness. Synthetic
fixture logic is covered by the exact Node test rather than published into the Pages
artifact.

No successful organization merge, Actions deployment, DNS, legal approval or public
contact is claimed by this note until separately witnessed.

## next review question

can the display organ metabolize its first actual encounter nutrients into living
visitor-facing tissue without prematurely inventing an internal taxonomy—and can the
host orientation interface expose siblings / parent / children without making that
navigation physiology into display's semantic constitution?
