...
you are entering the website organism of self-similar-systems saar.
...
## current host constitution

- fixed host DNA remains `w=CREATE · x=COPY · z=CONTROL · y=CULTIVATE`.
- `INDEX.yaml` is phenotype only; `_cambium.yaml` exists only for closed splits and
  contains exactly the surviving `4V/6E/4F/1T`.
...
optional browser witness:

```sh
python3 y/browser-check.py --out ../cambium-review
```

before remote writes reacquire the staging head and preserve concurrent work.
`QuantumCephalopod/cambium` is the writable staging fork. Fork mutation becomes
organization truth only after deliberate owner merge into `self-similar-systems/cambium`
and provider readback witnesses the resulting tree/deployment.
